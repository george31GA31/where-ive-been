...
(function(root) {
  ...
  function visits(state,category,today,profileId=state.activeProfileId) {
    ...
    if(category==='airports') scoped(state.transports,profileId).filter(t=>t.type==='flight'&&isActual(t)).forEach(t=>{
  ...
  const api={categories,types,scoped,isActual,countsForPlanning,reviewPlanned,isHome,dayStatus,validDate,validLocal,validateTransport,visits,routeColor:type=>type==='flight'?'#66DCE3':'#74F94B'};
