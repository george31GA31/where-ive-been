...
const RULES=[
 {id:'schengen',name:'Schengen Area',countries:[...SCHENGEN],type:'rolling',limit:90,window:180,source:'https://home-affairs.ec.europa.eu/policies/schengen/border-crossing/short-stay-calculator_en',desc:'90 days in any rolling 180-day period for normal short stays by third-country nationals.'},
 {id:'uk',name:'United Kingdom — Standard Visitor',countries:['GB'],type:'months',months:6,source:'https://www.gov.uk/standard-visitor',desc:'A Standard Visitor can usually stay for up to 6 months. Your actual permission and eligibility control.'},
 {id:'canada',name:'Canada — visitor',countries:['CA'],type:'months',months:6,source:'https://www.ircc.canada.ca/english/helpcentre/answer.asp?qnum=1016&top=',desc:'Most visitors can stay for up to 6 months, but a border officer may allow a different period.'},
...
function defaultState(){return{version:2,stays:[],profiles:[{id:uid(),name:'Me',citizenships:[],enabledRules:['schengen']}],activeProfileId:null,excludedCountryCodes:[]};}
...
function normalizeState(v){v.version=2;v.stays=Array.isArray(v.stays)?v.stays:[];v.profiles=Array.isArray(v.profiles)&&v.profiles.length?v.profiles:defaultState().profiles;v.activeProfileId=v.activeProfileId||v.profiles[0].id;v.excludedCountryCodes=Array.isArray(v.excludedCountryCodes)?v.excludedCountryCodes:[];v.profiles.forEach(p=>{p.citizenships=p.citizenships||[];p.enabledRules=p.enabledRules||['schengen']});v.stays.forEach(s=>{s.status=s.status||'actual';if(s.profileId===undefined)s.profileId=null})}
function persist(noCloud=false){localStorage.setItem(APP_KEY,JSON.stringify(state));localStorage.setItem(LOCAL_SAVED_KEY,new Date().toISOString());updateLocalSaveIndicator();if(!noCloud&&cloudSession&&getCloudConfig().autoSync){clearTimeout(cloudTimer);cloudTimer=setTimeout(()=>cloudPush(true),700)}}
...
function flag(c){return c?[...c.toUpperCase()].map(x=>String.fromCodePoint(127397+x.charCodeAt())).join(''):''}
function flagUrl(c,w=80){return c?`https://flagcdn.com/w${w}/${String(c).toLowerCase()}.png`:''}
function flagHtml(c,cls='flag-img'){if(!c)return'';let code=String(c).toUpperCase();return`<img class="${esc(cls)}" src="${flagUrl(code)}" alt="${code} flag" loading="lazy" onerror="this.style.display='none'">`}
...
function datesForStay(s,min=null,max=null){let a=parseDate(s.start),b=parseDate(s.end),lo=min?parseDate(min):a,hi=max?parseDate(max):b,start=a>lo?a:lo,end=b<hi?b:hi,out=[];for(let d=start;d<=end;d=addDays(d,1))out.push(dayKey(d));return out}
function travelDaySet(list=state.stays){let set=new Set();list.forEach(s=>datesForStay(s).forEach(d=>set.add(d)));return set}
function schengenDaySet(min=null,max=null,profileId=state.activeProfileId,extra=[]){let set=new Set();[...staysForProfile(profileId),...extra].forEach(s=>{if(SCHENGEN.has(s.countryCode)&&!s.schengenExempt)datesForStay(s,min,max).forEach(d=>set.add(d))});return set}
function rollingStatus(check,profileId=state.activeProfileId,extra=[]){let end=parseDate(check),start=addDays(end,-179),set=schengenDaySet(dayKey(start),dayKey(end),profileId,extra),used=set.size;return{start,end,used,remaining:90-used}}
function statusForUsed(n){if(n>90)return{kind:'bad',label:'OVER',text:`Your records show ${n} Schengen days in this rolling 180-day window.`};if(n>=86)return{kind:'bad',label:'VERY CLOSE',text:`Only ${90-n} Schengen days remain in this rolling window.`};if(n>=75)return{kind:'warn',label:'WATCH',text:`You have ${90-n} Schengen days remaining in this rolling window.`};return{kind:'good',label:'OK',text:`You have ${90-n} Schengen days remaining in this rolling window.`}}
function firstOverstay(from,horizon=500,profileId=state.activeProfileId,extra=[]){let d=parseDate(from);for(let i=0;i<=horizon;i++,d=addDays(d,1)){let r=rollingStatus(dayKey(d),profileId,extra);if(r.used>90)return{date:dayKey(d),used:r.used}}return null}
function maxContinuousSchengen(entry,profileId=state.activeProfileId){let last=null;for(let i=0;i<180;i++){let d=dayKey(addDays(parseDate(entry),i)),trial={id:'planner',countryCode:'FR',countryName:'Schengen',start:entry,end:d,schengenExempt:false,status:'planned',profileId};let r=rollingStatus(d,profileId,[trial]);if(r.used>90)break;last=d}return last?daysInclusive(entry,last):0}
function earliestOneDayEntry(from,profileId=state.activeProfileId,extra=[]){let d=parseDate(from);for(let i=0;i<730;i++,d=addDays(d,1)){let k=dayKey(d),trial={id:'reentry',countryCode:'FR',countryName:'Schengen',start:k,end:k,schengenExempt:false,status:'planned',profileId};if(rollingStatus(k,profileId,[...extra,trial]).used<=90)return k}return null}
function findGaps(){let source=staysForProfile(state.activeProfileId);if(source.length<2)return[];let arr=[...source].sort((a,b)=>a.start.localeCompare(b.start)||a.end.localeCompare(b.end)),gaps=[],curEnd=arr[0].end,prev=arr[0];for(let i=1;i<arr.length;i++){let s=arr[i];if(parseDate(s.start)>addDays(parseDate(curEnd),1)){let missingStart=dayKey(addDays(parseDate(curEnd),1)),missingEnd=dayKey(addDays(parseDate(s.start),-1)),gs=curEnd,ge=s.start;gaps.push({start:gs,end:ge,days:daysInclusive(gs,ge),missingStart,missingEnd,missingDays:daysInclusive(missingStart,missingEnd),prev,next:s})}if(s.end>curEnd){curEnd=s.end;prev=s}}return gaps}
...
function renderCountries(){if(!state.stays.length){els.countryTotals.className='country-totals empty-state';els.countryTotals.textContent='No country data yet.';return}let m=new Map();state.stays.forEach(s=>{if(!m.has(s.countryCode))m.set(s.countryCode,{name:s.countryName,days:new Set(),actual:new Set(),planned:new Set(),count:0});let x=m.get(s.countryCode);x.count++;datesForStay(s).forEach(d=>{x.days.add(d);x[s.status==='planned'?'planned':'actual'].add(d)})});let allRows=[...m].map(([code,x])=>({code,...x,total:x.days.size})).sort((a,b)=>b.total-a.total),excluded=new Set(state.excludedCountryCodes||[]),rows=allRows.filter(r=>!excluded.has(r.code)),hidden=allRows.filter(r=>excluded.has(r.code)),max=rows.length?Math.max(...rows.map(r=>r.total)):1;els.countryTotals.className='country-totals';let visibleHtml=rows.length?rows.map(r=>`<div class="country-row"><div class="flag">${flagHtml(r.code)}</div><div class="country-name"><strong>${esc(r.name)}</strong><span>${r.actual.size} actual${r.planned.size?` · ${r.planned.size} planned`:''}</span><button type="button" class="text-btn" data-action="exclude-country" data-country="${r.code}" style="margin-top:4px;color:#8b95a4">Remove from totals</button></div><div class="country-bar"><span style="width:${Math.max(0,Math.min(100,r.total/max*100))}%"></span></div><div class="country-count"><strong>${r.total}</strong><span>days</span></div></div>`).join(''):'<div class="empty-state">All recorded countries are currently removed from totals.</div>';let hiddenHtml=hidden.length?`<div class="gap-card" style="margin-top:14px"><div class="gap-card-head"><strong>Removed from country totals</strong><span class="status-badge neutral">${hidden.length}</span></div><p>These stays are still saved in your calendar and travel history. Re-add a country at any time.</p><div class="gap-actions">${hidden.map(r=>`<button type="button" class="tiny-btn" data-action="include-country" data-country="${r.code}">${flagHtml(r.code,'flag-img flag-sm')} Re-add ${esc(r.name)}</button>`).join('')}</div></div>`:'';els.countryTotals.innerHTML=visibleHtml+hiddenHtml}

function schengenBreakdownGroups(min,max,profileId=state.activeProfileId){let dayMap=new Map();staysForProfile(profileId).forEach(s=>{if(!SCHENGEN.has(s.countryCode)||s.schengenExempt)return;datesForStay(s,min,max).forEach(d=>{if(!dayMap.has(d))dayMap.set(d,new Map());let countries=dayMap.get(d),rec=countries.get(s.countryCode)||{code:s.countryCode,name:s.countryName,statuses:new Set()};rec.statuses.add(s.status);countries.set(s.countryCode,rec)})});let groups=new Map();[...dayMap.entries()].sort((a,b)=>a[0].localeCompare(b[0])).forEach(([date,countries])=>{let list=[...countries.values()].sort((a,b)=>a.code.localeCompare(b.code)),key=list.map(c=>c.code).join('+'),g=groups.get(key)||{countries:list,dates:[],hasPlanned:false};g.dates.push(date);if(list.some(c=>c.statuses.has('planned')))g.hasPlanned=true;groups.set(key,g)});return[...groups.values()].sort((a,b)=>b.dates.length-a.dates.length||a.countries.length-b.countries.length)}
function compactBreakdownDates(dates){if(!dates.length)return'';if(dates.length===1)return fmt(dates[0],{day:'numeric',month:'short'});if(dates.length<=3)return dates.map(d=>fmt(d,{day:'numeric',month:'short'})).join(', ');return`${fmt(dates[0],{day:'numeric',month:'short'})} … ${fmt(dates[dates.length-1],{day:'numeric',month:'short'})}`}
function renderSchengen(){let p=activeProfile(),check=els.checkDate.value||isoDate(new Date()),exempt=isSchengenExemptProfile(p);els.schengenProfileNote.textContent=exempt?`${p.name} has an EU/EEA/Swiss citizenship recorded, so this short-stay tracker is not applied to this profile.`:`For ${p.name}: choose any date to count Schengen days in that date and the previous 179 days. A calendar date counts once even if you visit more than one Schengen country that day.`;if(exempt){els.ringUsed.textContent='—';els.calcUsed.textContent='Not applied';els.calcRemaining.textContent='—';els.windowDates.textContent='Free-movement profile';els.forecastBox.className='forecast-box good-box';els.forecastTitle.textContent='90/180 tracker disabled for this profile';els.forecastText.textContent='Local registration or residence requirements can still apply.';els.schengenBreakdown.className='breakdown empty-state';els.schengenBreakdown.textContent='Switch to a non-EU/EEA/Swiss traveller to use the short-stay calculator.';return}let r=rollingStatus(check),st=statusForUsed(r.used);els.ringUsed.textContent=r.used;els.calcUsed.textContent=plural(r.used,'day');els.calcRemaining.textContent=plural(Math.max(0,r.remaining),'day');els.windowDates.textContent=`${fmtObj(r.start,{day:'numeric',month:'short',year:'2-digit'})} – ${fmtObj(r.end,{day:'numeric',month:'short',year:'2-digit'})}`;els.schengenRing.style.setProperty('--p',`${Math.min(360,r.used/90*360)}deg`);let col=st.kind==='bad'?'var(--red)':st.kind==='warn'?'var(--amber)':'var(--green)';els.schengenRing.style.background=`radial-gradient(circle at center,#fff 58%,transparent 59%),conic-gradient(${col} var(--p),#edf0f3 0)`;let future=firstOverstay(check);els.forecastBox.className='forecast-box '+(r.used>90?'bad-box':future?'warn-box':'good-box');els.forecastTitle.textContent=r.used>90?'Possible overstay in your records':future?'A future recorded stay may cross the limit':'No recorded overstay';els.forecastText.textContent=r.used>90?`This date has ${r.used} countable Schengen days in its rolling window.`:future?`Your current records first exceed 90 days on ${fmt(future.date)}.`:'Your currently recorded stays remain within the rule over the next 500 days.';let groups=schengenBreakdownGroups(dayKey(r.start),dayKey(r.end),p.id);els.schengenBreakdown.className=groups.length?'breakdown':'breakdown empty-state';els.schengenBreakdown.innerHTML=groups.length?groups.map(g=>{let overlap=g.countries.length>1,names=g.countries.map(c=>esc(c.name)).join(' + '),flags=g.countries.map(c=>flagHtml(c.code,'flag-img flag-sm')).join(' '),dateText=compactBreakdownDates(g.dates),detail=overlap?`${dateText} · multi-country ${g.dates.length===1?'travel day':'travel days'} · counted once ${g.dates.length===1?'':'each'}`:`${dateText}${g.hasPlanned?' · includes planned':''}`;return`<div class="breakdown-row"><div class="flag">${flags}</div><div><strong>${overlap?'Multi-country: ':''}${names}</strong><span>${detail}</span></div><div class="breakdown-days">${g.dates.length}d</div></div>`}).join(''):'No Schengen days in this period.'}

function renderStayLists(){let sorted=[...state.stays].sort((a,b)=>b.start.localeCompare(a.start)||b.end.localeCompare(a.end));renderStayList(els.allStays,sorted,true);let gaps=findGaps();els.gapList.className=gaps.length?'gap-list':'gap-list empty-state';els.gapList.innerHTML=gaps.length?gaps.map(g=>gapCard(g,false)).join(''):'No gaps detected.'}
function renderStayList(container,list){if(!list.length){container.className='stay-list empty-state';container.textContent='No stays added yet.';return}container.className='stay-list';container.innerHTML=list.map(s=>`<div class="stay-row ${s.status==='planned'?'planned':''}"><div class="flag-box">${flagHtml(s.countryCode)}</div><div class="stay-main"><strong>${esc(s.countryName)} ${s.status==='planned'?'<span class="pill planned">PLANNED</span>':''}</strong><span>${fmt(s.start)} – ${fmt(s.end)}${s.notes?` · ${esc(s.notes)}`:''}</span></div><div class="stay-days"><strong>${daysInclusive(s.start,s.end)}</strong><span>days</span></div><button class="edit-btn" data-action="edit-stay" data-id="${s.id}">⋯</button></div>`).join('')}
function gapCard(g,compact){let prev=g.prev?.countryCode,next=g.next?.countryCode;return`<div class="gap-card"><div class="gap-card-head"><strong>${fmt(g.start)} – ${fmt(g.end)}</strong><span class="status-badge neutral">${plural(g.missingDays??Math.max(0,g.days-2),'unlogged day')}</span></div><p>Between ${g.prev?flagHtml(prev,'flag-img flag-sm')+' '+esc(g.prev.countryName):'record'} and ${g.next?flagHtml(next,'flag-img flag-sm')+' '+esc(g.next.countryName):'record'}. The fill range includes both boundary travel days, so those dates can contain two countries.</p><div class="gap-actions">${prev?`<button class="tiny-btn" data-action="fill-gap" data-start="${g.start}" data-end="${g.end}" data-country="${prev}">Use ${flagHtml(prev,'flag-img flag-sm')} ${esc(g.prev.countryName)}</button>`:''}${next&&next!==prev?`<button class="tiny-btn" data-action="fill-gap" data-start="${g.start}" data-end="${g.end}" data-country="${next}">Use ${flagHtml(next,'flag-img flag-sm')} ${esc(g.next.countryName)}</button>`:''}<button class="tiny-btn" data-action="fill-gap" data-start="${g.start}" data-end="${g.end}">Choose country</button></div></div>`}
function addDateRange(start=isoDate(new Date()),end=start){let row=document.createElement('div');row.className='date-range-row';row.innerHTML=`<label><span>From</span><input type="date" class="range-start" value="${start}" required></label><label><span>To</span><input type="date" class="range-end" value="${end}" required></label><button type="button" class="remove-range" data-action="remove-range" aria-label="Remove date range">×</button>`;let validate=()=>{let st=row.querySelector('.range-start').value,en=row.querySelector('.range-end').value;row.classList.toggle('invalid-range',!!st&&!!en&&en<st)};row.querySelector('.range-start').onchange=validate;row.querySelector('.range-end').onchange=validate;els.dateRanges.appendChild(row)}
function openStayDialog(id=null,prefill={}){stayDialogContext=prefill.source||'manual';els.stayForm.reset();els.dateRanges.innerHTML='';els.formError.textContent='';els.stayId.value=id||'';els.deleteStayBtn.classList.toggle('hidden',!id);let s=id?state.stays.find(x=>x.id===id):null;els.dialogTitle.textContent=id?'Edit stay':stayDialogContext==='calendar'?'Add selected dates':'Add a stay';els.countryInput.value=s?.countryName||prefill.countryName||'';els.notesInput.value=s?.notes||'';els.stayStatus.value=s?.status||prefill.status||'actual';els.stayProfile.value=s?.profileId||state.activeProfileId||'';els.schengenExempt.checked=!!s?.schengenExempt;addDateRange(s?.start||prefill.start||isoDate(new Date()),s?.end||prefill.end||prefill.start||isoDate(new Date()));updateStayCountry();els.stayDialog.showModal()}
function updateStayCountry(){let c=countryByName(els.countryInput.value);els.countryFlag.innerHTML=c?flagHtml(c.code):'';els.schengenExemptRow.classList.toggle('hidden',!c||!SCHENGEN.has(c.code))}
function saveStay(e){e.preventDefault();let c=countryByName(els.countryInput.value);if(!c){els.formError.textContent='Choose a country from the list.';return}let rows=[...els.dateRanges.querySelectorAll('.date-range-row')].map(r=>({start:r.querySelector('.range-start').value,end:r.querySelector('.range-end').value}));if(!rows.length){els.formError.textContent='Add at least one date range.';return}for(let r of rows)if(!r.start||!r.end||r.end<r.start){els.formError.textContent='Check each From/To date range. The To date must be on or after the From date.';return}let base={countryCode:c.code,countryName:c.name,notes:els.notesInput.value.trim(),schengenExempt:SCHENGEN.has(c.code)?els.schengenExempt.checked:false,status:els.stayStatus.value,profileId:els.stayProfile.value||null};let id=els.stayId.value;if(id){let ix=state.stays.findIndex(s=>s.id===id);state.stays[ix]={...state.stays[ix],...base,...rows[0]};rows.slice(1).forEach(r=>state.stays.push({id:uid(),...base,...r}))}else rows.forEach(r=>state.stays.push({id:uid(),...base,...r}));if(stayDialogContext==='calendar'){calendarSelectionStart=null;calendarSelectionEnd=null}stayDialogContext='manual';persist();els.stayDialog.close();renderAll()}
function deleteStay(){let id=els.stayId.value;if(!id)return;state.stays=state.stays.filter(s=>s.id!==id);persist();els.stayDialog.close();renderAll()}
function openGapDialog(start,end,code=''){els.gapStart.value=start;els.gapEnd.value=end;els.gapDateSummary.textContent=`Fill ${fmt(start)} to ${fmt(end)} (${plural(daysInclusive(start,end),'day')}). The first and last dates are intentionally included so travel days can overlap with the country either side.`;els.gapCountry.value=countryByCode(code)?.name||'';els.gapError.textContent='';els.gapDialog.showModal()}
function saveGap(e){e.preventDefault();let c=countryByName(els.gapCountry.value);if(!c){els.gapError.textContent='Choose a country from the list.';return}state.stays.push({id:uid(),countryCode:c.code,countryName:c.name,start:els.gapStart.value,end:els.gapEnd.value,notes:'Filled from gap detector',schengenExempt:false,status:'actual',profileId:state.activeProfileId});persist();els.gapDialog.close();renderAll()}
function populateProfileSelects(){let opts=state.profiles.map(p=>`<option value="${p.id}" ${p.id===state.activeProfileId?'selected':''}>${esc(p.name)}</option>`).join('');els.plannerProfile.innerHTML=opts;els.stayProfile.innerHTML='<option value="">All / shared</option>'+opts}
function renderProfiles(){let p=activeProfile();els.profileList.innerHTML=state.profiles.map(x=>`<div class="profile-card ${x.id===p.id?'active':''}"><div><h3>${esc(x.name)} ${x.id===p.id?'<span class="pill">ACTIVE</span>':''}</h3><p>${x.citizenships.length?x.citizenships.map(c=>flagHtml(c,'flag-img flag-sm')+' '+esc(countryByCode(c)?.name||c)).join(' · '):'No citizenship added yet'}</p></div><div class="profile-actions">${x.id!==p.id?`<button class="tiny-btn" data-action="set-profile" data-id="${x.id}">Use</button>`:''}<button class="tiny-btn" data-action="edit-profile" data-id="${x.id}">Edit</button></div></div>`).join('');populateProfileSelects()}
function openProfileDialog(id=null){let p=id?state.profiles.find(x=>x.id===id):null;els.profileForm.reset();els.profileId.value=id||'';els.profileName.value=p?.name||'';profileCitizenships=[...(p?.citizenships||[])];els.profileDialogTitle.textContent=id?'Edit profile':'Add profile';els.deleteProfileBtn.classList.toggle('hidden',!id||state.profiles.length===1);els.profileError.textContent='';renderCitizenshipChips();els.profileDialog.showModal()}
function addCitizenship(){let c=countryByName(els.citizenshipInput.value);if(!c){els.profileError.textContent='Choose a country from the list.';return}if(!profileCitizenships.includes(c.code))profileCitizenships.push(c.code);els.citizenshipInput.value='';els.profileError.textContent='';renderCitizenshipChips()}
function renderCitizenshipChips(){els.citizenshipChips.innerHTML=profileCitizenships.map(c=>`<span class="chip">${flagHtml(c,'flag-img flag-sm')} ${esc(countryByCode(c)?.name||c)} <button type="button" data-remove-cit="${c}">×</button></span>`).join('');els.citizenshipChips.querySelectorAll('[data-remove-cit]').forEach(b=>b.onclick=()=>{profileCitizenships=profileCitizenships.filter(c=>c!==b.dataset.removeCit);renderCitizenshipChips()})}
function saveProfile(e){e.preventDefault();let name=els.profileName.value.trim();if(!name){els.profileError.textContent='Add a name.';return}let id=els.profileId.value;if(id){let p=state.profiles.find(x=>x.id===id);p.name=name;p.citizenships=[...profileCitizenships]}else{let p={id:uid(),name,citizenships:[...profileCitizenships],enabledRules:['schengen']};state.profiles.push(p);state.activeProfileId=p.id}persist();els.profileDialog.close();renderAll()}
function deleteProfile(){let id=els.profileId.value;if(state.profiles.length<=1)return;state.profiles=state.profiles.filter(p=>p.id!==id);state.stays.forEach(s=>{if(s.profileId===id)s.profileId=null});if(state.activeProfileId===id)state.activeProfileId=state.profiles[0].id;persist();els.profileDialog.close();renderAll()}
...
async function runVisaCheck(){
  ...
  els.visaResultTitle.textContent='Checking…';els.visaResultBody.innerHTML='<div class="planner-placeholder">Loading the visa-policy data.</div>';

  if(from.code===to.code){
    els.visaResultTitle.innerHTML=`${flagHtml(from.code)} ${esc(from.name)} → ${flagHtml(to.code)} ${esc(to.name)}`;
    els.visaResultBody.innerHTML=`<div class="visa-result-card"><div class="visa-answer good"><span class="status-badge good">YOUR COUNTRY</span><h3>No entry visa needed as a citizen</h3><p>This checker is for foreign-entry visa requirements. You may still need a valid passport or national identity document for travel.</p></div></div>`;
    return;
  }
  ...
  try{
    ...
    els.visaResultBody.innerHTML=`<div class="visa-result-card"><div class="visa-route"><div class="route-country">${flagHtml(from.code,'flag-img flag-lg')}<span>${esc(from.name)} passport</span></div><span class="route-arrow">→</span><div class="route-country">${flagHtml(to.code,'flag-img flag-lg')}<span>${esc(to.name)}</span></div></div><div class="visa-answer ${info.tone}"><span class="status-badge ${info.tone==='bad'?'bad':info.tone==='warn'?'warn':info.tone==='good'?'good':'neutral'}">${esc(info.badge)}</span><h3>${esc(info.title)}</h3><p>For a short tourist visit, the quick dataset lists this route as <strong>${esc(info.label)}</strong>${rule.days?` with a stay of up to <strong>${rule.days} days</strong>`:''}. Check the live rule before you travel.</p></div><div class="visa-detail-grid"><div class="visa-detail"><span>Do I need a visa?</span><strong>${esc(info.answer)}</strong></div><div class="visa-detail"><span>Quick requirement</span><strong>${esc(info.label)}</strong></div><div class="visa-detail"><span>Stay shown</span><strong>${esc(stay)}</strong></div></div><div class="visa-actions"><a class="primary" href="https://apply.joinsherpa.com/visa" target="_blank" rel="noreferrer">Verify live requirements ↗</a></div></div>`;
     }catch(err){
    els.visaResultTitle.textContent=`${from.name} → ${to.name}`;
    els.visaResultBody.innerHTML=`<div class="visa-error">I couldn't get a reliable quick result for this route. Use the live requirements checker below before relying on an answer.</div><div class="visa-actions" style="margin-top:10px"><a class="primary" href="https://apply.joinsherpa.com/visa" target="_blank" rel="noreferrer">Open live requirements ↗</a></div>`;
  }
}

function visitBlocksForRule(rule,profileId){let dates=new Set();staysForProfile(profileId).filter(s=>rule.countries.includes(s.countryCode)).forEach(s=>datesForStay(s).forEach(d=>dates.add(d)));let a=[...dates].sort(),out=[];for(let d of a){let last=out.at(-1);if(!last||diffDays(last.end,d)>1)out.push({start:d,end:d,days:1});else{last.end=d;last.days++}}return out}

function runPlanner(){let p=state.profiles.find(x=>x.id===els.plannerProfile.value)||activeProfile(),c=countryByName(els.plannerCountry.value),entry=els.plannerEntry.value,exit=els.plannerExit.value;lastPlannerTrip=null;els.addPlannedTripBtn.classList.add('hidden');if(!c||!entry){els.plannerResultTitle.textContent='Add a destination and entry date';els.plannerResultBody.innerHTML='<div class="planner-placeholder">Choose a valid country and date.</div>';return}if(exit&&exit<entry){els.plannerResultTitle.textContent='Check your dates';els.plannerResultBody.innerHTML='<div class="result-message bad">Departure cannot be before entry.</div>';return}if(SCHENGEN.has(c.code)){if(isSchengenExemptProfile(p)){els.plannerResultTitle.textContent='Schengen 90/180 does not apply to this profile';els.plannerResultBody.innerHTML='<div class="result-message">An EU/EEA/Swiss citizenship is recorded for this traveller. Free-movement and local registration/residence rules are different from the visitor 90/180 rule.</div>';return}let max=maxContinuousSchengen(entry,p.id),safeEnd=max?dayKey(addDays(parseDate(entry),max-1)):null,tripExit=exit||safeEnd,trial=tripExit?{id:'planner',countryCode:c.code,countryName:c.name,start:entry,end:tripExit,schengenExempt:false,status:'planned',profileId:p.id}:null,over=trial?firstOverstay(entry,Math.max(1,daysInclusive(entry,tripExit)),p.id,[trial]):null,reentry=safeEnd?earliestOneDayEntry(dayKey(addDays(parseDate(safeEnd),1)),p.id,[{id:'usedmax',countryCode:c.code,countryName:c.name,start:entry,end:safeEnd,schengenExempt:false,status:'planned',profileId:p.id}]):earliestOneDayEntry(entry,p.id);els.plannerResultTitle.innerHTML=`${flagHtml(c.code)} ${esc(c.name)} · Schengen`;let ok=!exit||!over;els.plannerResultBody.innerHTML=`<div class="result-hero"><div class="result-stat"><span>MAX CONTINUOUS STAY</span><strong>${max} days</strong></div><div class="result-stat"><span>SAFE DEPARTURE</span><strong>${safeEnd?fmt(safeEnd,{day:'numeric',month:'short'}):'Not available'}</strong></div><div class="result-stat"><span>RE-ENTRY AFTER USING MAX</span><strong>${reentry?fmt(reentry,{day:'numeric',month:'short'}):'—'}</strong></div></div><div class="result-message ${ok?'':'bad'}">${exit?(over?`Your proposed trip would first exceed the 90/180 limit on <strong>${fmt(over.date)}</strong>. Shorten the trip or change the dates.`:`Your proposed ${daysInclusive(entry,exit)}-day trip stays within the recorded 90/180 history.`):max?`If you enter on <strong>${fmt(entry)}</strong>, your current records allow up to <strong>${max} consecutive days</strong>, leaving by <strong>${fmt(safeEnd)}</strong>.`:`Your recorded history does not leave a Schengen day available on ${fmt(entry)}. Earliest one-day entry found: <strong>${reentry?fmt(reentry):'not within the next 2 years'}</strong>.`}</div>`;if(tripExit&&ok){lastPlannerTrip={country:c,start:entry,end:tripExit,profileId:p.id};els.addPlannedTripBtn.classList.remove('hidden')}return}let rule=RULES.find(r=>r.countries.includes(c.code));els.plannerResultTitle.innerHTML=`${flagHtml(c.code)} ${esc(c.name)}`;if(rule){let leave=rule.months?dayKey(addDays(addMonths(parseDate(entry),rule.months),-1)):rule.limit?dayKey(addDays(parseDate(entry),rule.limit-1)):null;els.plannerResultBody.innerHTML=`<div class="result-hero"><div class="result-stat"><span>TRACKER TEMPLATE</span><strong>${rule.months?`Up to ${rule.months} months`:`${rule.limit} days`}</strong></div><div class="result-stat"><span>PLANNING DATE</span><strong>${leave?fmt(leave,{day:'numeric',month:'short'}):'—'}</strong></div></div><div class="result-message warn">${esc(rule.desc)} This is only a planning template; verify that you are eligible and check the actual permission granted to you.</div>`}else els.plannerResultBody.innerHTML='<div class="result-message">No built-in visa-day template is attached to this country yet. You can still add the trip to your travel history.</div>';if(exit){lastPlannerTrip={country:c,start:entry,end:exit,profileId:p.id};els.addPlannedTripBtn.classList.remove('hidden')}}

function addPlannerTrip(){if(!lastPlannerTrip)return;let t=lastPlannerTrip;state.stays.push({id:uid(),countryCode:t.country.code,countryName:t.country.name,start:t.start,end:t.end,notes:'Added from trip planner',schengenExempt:false,status:'planned',profileId:t.profileId});persist();renderAll();els.addPlannedTripBtn.classList.add('hidden');els.plannerResultBody.insertAdjacentHTML('beforeend','<div class="form-message" style="color:var(--green)">Planned trip added to your calendar.</div>')}

function renderMapTimeline(){let list=state.stays;if(!list.length){els.timelineEmpty.classList.remove('hidden');els.timelineContent.classList.add('hidden');renderWorldMap();return}els.timelineEmpty.classList.add('hidden');els.timelineContent.classList.remove('hidden');let min=list.reduce((a,s)=>s.start<a?s.start:a,list[0].start),max=list.reduce((a,s)=>s.end>a?s.end:a,list[0].end),today=isoDate(new Date());if(today<min)min=today;if(today>max)max=today;let total=Math.max(1,diffDays(min,max));els.timelineSlider.max=total;timelineDate=timelineDate&&timelineDate>=min&&timelineDate<=max?timelineDate:(today>=min&&today<=max?today:max);els.timelineSlider.value=diffDays(min,timelineDate);els.timelineSlider.dataset.start=min;els.timelineStartLabel.textContent=fmt(min,{month:'short',year:'numeric'});els.timelineEndLabel.textContent=fmt(max,{month:'short',year:'numeric'});els.timelineBars.innerHTML=list.map((s,i)=>{let l=diffDays(min,s.start)/total*100,w=Math.max(.35,daysInclusive(s.start,s.end)/(total+1)*100),top=(i%3)*21+7;return`<div class="timeline-bar ${s.status==='planned'?'planned':''}" style="left:${l}%;width:${w}%;top:${top}px" title="${esc(s.countryName)}: ${fmt(s.start)} – ${fmt(s.end)}"></div>`}).join('');updateTimelineLabels();renderWorldMap()}
function setTimelineFromSlider(){let start=els.timelineSlider.dataset.start;timelineDate=dayKey(addDays(parseDate(start),Number(els.timelineSlider.value)));updateTimelineLabels();updateMapColors()}
function setTimelineDate(k){if(!state.stays.length)return;let start=els.timelineSlider.dataset.start,max=Number(els.timelineSlider.max),v=Math.max(0,Math.min(max,diffDays(start,k)));els.timelineSlider.value=v;setTimelineFromSlider()}
function updateTimelineLabels(){if(!timelineDate)return;let on=state.stays.filter(s=>s.start<=timelineDate&&s.end>=timelineDate);els.timelineDateLabel.textContent=fmt(timelineDate,{weekday:'long',day:'numeric',month:'long',year:'numeric'});els.timelineLocationLabel.textContent=on.length?on.map(s=>`${s.countryName}${s.status==='planned'?' (planned)':''}`).join(' · '):'No country recorded on this date';let r=rollingStatus(timelineDate);els.timelineSchengenLabel.textContent=isSchengenExemptProfile()? 'Not applied' : `${r.used} / 90`;els.mapSelectionSummary.textContent=on.length?`On ${fmt(timelineDate)}, you have ${on.map(s=>s.countryName).join(' / ')} recorded.`:`No stay is recorded on ${fmt(timelineDate)}.`}

async function renderWorldMap(){if(!window.d3||!window.topojson){els.mapFallback.classList.remove('hidden');return}if(!worldFeatures&&!worldLoading){worldLoading=true;try{let res=await fetch('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json');if(!res.ok)throw Error('map');let world=await res.json();worldFeatures=topojson.feature(world,world.objects.countries).features}catch{els.mapFallback.classList.remove('hidden')}finally{worldLoading=false}}if(!worldFeatures)return;els.mapFallback.classList.add('hidden');let svg=d3.select(els.worldMap);svg.selectAll('*').remove();let fc={type:'FeatureCollection',features:worldFeatures},proj=d3.geoNaturalEarth1().fitExtent([[8,8],[992,492]],fc),path=d3.geoPath(proj);svg.selectAll('path').data(worldFeatures).enter().append('path').attr('d',path).attr('data-code',d=>NUMERIC_TO_ALPHA2[String(d.id).padStart(3,'0')]||'').attr('class','map-country').append('title').text(d=>countryByCode(NUMERIC_TO_ALPHA2[String(d.id).padStart(3,'0')])?.name||'');updateMapColors()}
function updateMapColors(){if(!worldFeatures)return;let asOf=timelineDate||isoDate(new Date()),actual=new Set(),planned=new Set(),current=new Set(),excluded=new Set(state.excludedCountryCodes||[]);state.stays.forEach(s=>{if(excluded.has(s.countryCode))return;if(s.start<=asOf){(s.status==='planned'?planned:actual).add(s.countryCode)}if(s.start<=asOf&&s.end>=asOf)current.add(s.countryCode)});d3.select(els.worldMap).selectAll('.map-country').attr('class',function(){let c=this.dataset.code;return`map-country${actual.has(c)?' visited':planned.has(c)?' planned':''}${current.has(c)?' current':''}`})}
  ...
  profiles.insertAdjacentHTML('beforeend',`
    <article class="panel data-transfer-panel" id="dataTransferPanel">
      ...
      <p class="panel-copy">
        ...
        No account is required. Use a transfer code to move
        everything to another device, or keep a backup file
        as extra protection.
      </p>
        ...
        <div class="transfer-card">
          ...
          <div id="transferCodeArea" class="transfer-code-area hidden">
            ...
            <div class="button-row">
              <button class="secondary" id="copyTransferCodeBtn" type="button">
                Copy code
              </button>
            </div>

            <small id="transferExpiry">
              Valid for 1 hour and can only be used once.
            </small>
          </div>
        </div>
...
function getTransferClient(){
  ...
  if(!cfg.valid)
    throw new Error('Device transfer has not been configured by the site owner yet.');

  if(!window.supabase)
    throw new Error('The transfer service is still loading. Try again in a moment.');

  if(!transferClient)
    transferClient=window.supabase.createClient(
      cfg.url,
      cfg.key,
      {
        auth:{
...
async function transferCryptoKey(code){
  let digest=await crypto.subtle.digest(
    ...
    new TextEncoder().encode(
      `where-ive-been-transfer:v1:${normalizeTransferCode(code)}`
    )
  );
...
async function decryptTransferPayload(code,encryptedPayload,ivB64){
  ...
  if(
    payload?.format!=='where-ive-been-transfer'||
    !payload.state
  )
...
function setTransferMessage(message,good=false){
  ...
  els.transferMessage.style.color=
    good?'var(--green)':'var(--red)';
}

function setTransferBusy(busy){
  if(els.createTransferBtn)
    els.createTransferBtn.disabled=busy;

  if(els.claimTransferBtn)
    els.claimTransferBtn.disabled=busy;
}
...
async function createTransferCode(){
  try{
    ...
    els.transferExpiry.textContent=
      expiry
    ...
    setTransferMessage(
      e?.message||
      'Could not create a transfer code.'
    );
  }
  finally{
...
async function claimTransferCode(){
  let raw=normalizeTransferCode(
    els.transferCodeInput?.value||''
  );

  if(raw.length!==16){
    setTransferMessage(
      'Enter the complete 16-character transfer code.'
    );
    return;
  }
  ...
  try{
    setTransferBusy(true);
    setTransferMessage(
      'Loading your encrypted travel data…',
      true
    );
    ...
    let incoming=await decryptTransferPayload(
      raw,
  ...
  r.onload=()=>{
    try{
      let parsed=JSON.parse(r.result),
          v=
            parsed?.format==='where-ive-been-backup'&&
            parsed.data
              ? parsed.data
              : parsed;
      ...
      alert(
        "That file is not a valid Where I've Been backup."
      );
    }
...
async function initCloudFromConfig(){
  ...
  try{
    ...
    setCloudUI(
      cloudSession?'Signed in':'Configured',
      !!cloudSession
    );
  }
  catch(e){
    setCloudMessage(
      'Could not connect to Supabase. Check the URL/key.'
    );
...
function setCloudUI(label,signed){
  ...
  els.cloudStatusBadge.className=
    `status-badge ${signed?'good':'neutral'}`;

  els.cloudPullBtn.disabled=!signed;
  els.cloudPushBtn.disabled=!signed;

  els.cloudSignInBtn.classList.toggle(
    'hidden',
    signed
  );

  els.cloudSignOutBtn.classList.toggle(
    'hidden',
    !signed
  );
}
...
async function cloudSignUp(){
  if(!cloudClient){
    setCloudMessage(
      'Save valid cloud settings first.'
    );
    return;
  }
  ...
  let{error}=await cloudClient.auth.signUp({
    email,
    password
  });
...
async function cloudSignIn(){
  ...
  let{data,error}=
    await cloudClient.auth.signInWithPassword({
  ...
  setCloudMessage(
    'Signed in. Use Pull or Push to choose which copy wins first.',
    true
  );
}
...
async function cloudPull(){
  ...
  if(!data?.payload){
    setCloudMessage(
      'No cloud copy exists yet. Push this device first.'
    );
    return;
  }
  ...
  setCloudMessage(
    'Cloud copy loaded onto this device.',
    true
  );
}
