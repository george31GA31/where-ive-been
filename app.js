...
function normalizeState(v){
  ...
  v.profiles=Array.isArray(v.profiles)&&v.profiles.length
    ?v.profiles
    :defaultState().profiles;

  v.activeProfileId=v.activeProfileId||v.profiles[0].id;

  v.excludedCountryCodes=Array.isArray(v.excludedCountryCodes)
    ?v.excludedCountryCodes
    :[];
...
function diffDays(a,b){
  return Math.round(
    (parseDate(b)-parseDate(a))/86400000
  );
}
...
function flagUrl(c,w=80){
  return c
    ?`https://flagcdn.com/w${w}/${String(c).toLowerCase()}.png`
    :'';
}
...
function isSchengenExemptProfile(
  p=activeProfile()
){
  return!!p&&
    p.citizenships.some(
...
function travelDaySet(
  list=state.stays
){
  let set=new Set();

  list.forEach(
    s=>datesForStay(s).forEach(
      d=>set.add(d)
    )
  );
...
function statusForUsed(n){
  if(n>90)
    return{
      ...
      label:'OVER',
      text:
        `Your records show ${n} Schengen days in this rolling 180-day window.`
    };
  ...
  if(n>=86)
    return{
      ...
      label:'VERY CLOSE',
      text:
        `Only ${90-n} Schengen days remain in this rolling window.`
    };
  ...
  if(n>=75)
    return{
      ...
      label:'WATCH',
      text:
        `You have ${90-n} Schengen days remaining in this rolling window.`
    };
...
function maxContinuousSchengen(
  entry,
  profileId=state.activeProfileId
){
  let last=null;
  ...
  return last
    ?daysInclusive(entry,last)
    :0;
}

function earliestOneDayEntry(
  from,
  profileId=state.activeProfileId,
  extra=[]
){
...
/* =========================================================
   AUTO-CONVERT PASSED PLANNED DAYS
   ========================================================= */
...
function updatePassedPlannedTrips(){
  ...
  state.stays.forEach(s=>{
    ...
    /*
     * If the entire planned trip
     * is already in the past,
     * simply make it actual.
     */
    ...
    /*
     * If part of the trip has passed,
     * split it into:
     *
     * actual: start → yesterday
     * planned: today → original end
     */
...
function init(){
  ...
  $('countryList').innerHTML=
    COUNTRIES.map(
      c=>`<option value="${esc(c.name)}"></option>`
    ).join('');

  els.checkDate.value=
    isoDate(new Date());

  els.plannerEntry.value=
    isoDate(new Date());
...
function bindEvents(){
  ...
  $('addDateRangeBtn').onclick=
    ()=>addDateRange();


  els.stayForm.onsubmit=
    saveStay;


  $('deleteStayBtn').onclick=
    deleteStay;


  els.countryInput.oninput=
    updateStayCountry;
  ...
  $('prevMonth').onclick=()=>{
    ...
    calendarCursor=
      new Date(
        Date.UTC(
          calendarCursor.getUTCFullYear(),
          calendarCursor.getUTCMonth()-1,
          1
        )
      );
  ...
  els.clearCalendarSelectionBtn.onclick=
    clearCalendarSelection;


  els.checkDate.onchange=
    renderSchengen;


  $('timelineTodayBtn').onclick=
    ()=>setTimelineDate(
      isoDate(new Date())
    );
  ...
  $('runPlannerBtn').onclick=
    runPlanner;


  if(els.runVisaCheckBtn)
    els.runVisaCheckBtn.onclick=
      runVisaCheck;


  if(els.visaPassport)
    els.visaPassport.addEventListener(
      'keydown',
      e=>{
        if(e.key==='Enter'){
          e.preventDefault();
  ...
  if(els.visaDestination)
    els.visaDestination.addEventListener(
      'keydown',
      e=>{
        if(e.key==='Enter'){
          e.preventDefault();
  ...
  $('closeProfileDialog').onclick=
  $('cancelProfileDialog').onclick=
    ()=>els.profileDialog.close();


  $('addCitizenshipBtn').onclick=
    addCitizenship;


  els.profileForm.onsubmit=
    saveProfile;
  ...
  els.gapForm.onsubmit=
    saveGap;


  $('closeGapDialog').onclick=
  $('cancelGapDialog').onclick=
    ()=>els.gapDialog.close();
  ...
  $('cloudSignUpBtn').onclick=
    cloudSignUp;


  $('cloudSignInBtn').onclick=
    cloudSignIn;


  els.cloudSignOutBtn.onclick=
    cloudSignOut;


  els.cloudPushBtn.onclick=
    ()=>cloudPush(false);


  els.cloudPullBtn.onclick=
    cloudPull;
  ...
  if(els.createTransferBtn)
    els.createTransferBtn.onclick=
      createTransferCode;


  if(els.claimTransferBtn)
    els.claimTransferBtn.onclick=
      claimTransferCode;


  if(els.copyTransferCodeBtn)
    els.copyTransferCodeBtn.onclick=
      copyTransferCode;


  if(els.transferCodeInput)
    els.transferCodeInput.addEventListener(
      'keydown',
      e=>{
        if(e.key==='Enter'){
          e.preventDefault();
  ...
  if(els.dataBackupBtn)
    els.dataBackupBtn.onclick=
      exportData;


  if(els.dataRestoreBtn)
    els.dataRestoreBtn.onclick=
      ()=>$('importInput').click();
}
...
function renderDashboard(){
  ...
  els.countriesLogged.textContent=
    new Set(
      state.stays
        .map(s=>s.countryCode)
        .filter(
  ...
  if(exempt){
    ...
    els.schengenBadge.className=
      'status-badge good';

    els.schengenBadge.textContent=
      'EXEMPT';

    els.schengenProgress.style.width=
      '0';

    els.schengenSummary.textContent=
      `${p.name} has an EU/EEA/Swiss citizenship recorded`;

    els.remainingSummary.textContent=
      '90/180 short-stay rule not applied';

    els.alertTitle.textContent=
      'Schengen 90/180 is not applied to this profile';

    els.alertBody.textContent=
      'EU/EEA/Swiss citizens use free-movement rules instead. Local registration/residence rules can still apply.';

    els.schengenAlertPanel.className=
      'panel warning-panel';
    ...
    els.schengenSummary.textContent=
      `${plural(r.used,'day')} used from ${fmtObj(r.start,{day:'numeric',month:'short'})} to ${fmtObj(r.end,{day:'numeric',month:'short'})}`;
  ...
  els.dashboardGaps.className=
    gaps.length
      ?'gap-list'
      :'gap-list empty-state';


  els.dashboardGaps.innerHTML=
    gaps.length
      ?gaps.map(
        g=>gapCard(g,true)
      ).join('')
      :'No gaps detected between your recorded stays.';
...
function renderMiniCalendar(){
  ...
  els.miniCalendarTitle.textContent=
  ...
  for(let i=0;i<42;i++){
    ...
    let d=addDays(start,i),
        ...
        selRange=
          ...
          k>=calendarSelectionStart&&
          k<=calendarSelectionEnd,
...
function updateCalendarSelectionUI(){
  ...
  if(
    !els.calendarSelectionInfo||
    !els.clearCalendarSelectionBtn
  )
    return;
  ...
  if(!calendarSelectionStart){
    ...
    info.innerHTML=
      '<strong>Click a date to add a stay</strong><span>Pick the first day, then the last day. Click the same day twice for a day trip.</span>';
  ...
  info.className=
    'calendar-selection-info complete';
  ...
  info.innerHTML=`
    <strong>
      ${fmt(calendarSelectionStart)}
      –
      ${fmt(calendarSelectionEnd)}
    </strong>

    <span>
      ${plural(days,'day')} selected.
      Cancel the pop-up to keep this selection,
      or clear it here.
    </span>
  `;
}
...
function renderCountries(){
  ...
  if(!state.stays.length){
    ...
    els.countryTotals.className=
      'country-totals empty-state';

    els.countryTotals.textContent=
      'No country data yet.';
    ...
    rows.length
      ...
      :`
        <div class="empty-state">
          All recorded countries are
          currently removed from totals.
        </div>
      `;
    ...
    hidden.length
      ?`
        ...
        >
          ...
          <p>
            These stays are still saved in
            your calendar and travel history.
            Re-add a country at any time.
          </p>
...
function renderSchengen(){
  ...
  let p=activeProfile(),
      check=
        ...
        isoDate(new Date()),
      exempt=
        isSchengenExemptProfile(p);


  els.schengenProfileNote.textContent=
    exempt
  ...
  if(exempt){
    ...
    els.windowDates.textContent=
      'Free-movement profile';

    els.forecastBox.className=
      'forecast-box good-box';

    els.forecastTitle.textContent=
      '90/180 tracker disabled for this profile';

    els.forecastText.textContent=
      'Local registration or residence requirements can still apply.';

    els.schengenBreakdown.className=
  ...
  els.schengenRing.style.setProperty(
    '--p',
    `${
      Math.min(
        360,
  ...
  els.schengenRing.style.background=
    `radial-gradient(circle at center,#fff 58%,transparent 59%),conic-gradient(${col} var(--p),#edf0f3 0)`;
  ...
  els.forecastText.textContent=
    r.used>90
      ?`This date has ${r.used} countable Schengen days in its rolling window.`
      :future
  ...
  els.schengenBreakdown.innerHTML=
    groups.length
      ?groups.map(g=>{
      ...
      <p>
        ...
        ${
          g.next
            ...
            ' '+
            esc(g.next.countryName)
            :'record'
        }.
        The fill range includes both boundary travel days,
        so those dates can contain two countries.
      </p>
...
function openStayDialog(
  ...
  els.deleteStayBtn.classList.toggle(
    'hidden',
    !id
  );
  ...
  els.stayProfile.value=
    s?.profileId||
    state.activeProfileId||
    '';


  els.schengenExempt.checked=
    !!s?.schengenExempt;
...
function updateStayCountry(){
  ...
  els.countryFlag.innerHTML=
    c
      ?flagHtml(c.code)
      :'';


  els.schengenExemptRow.classList.toggle(
    'hidden',
    !c||
    !SCHENGEN.has(c.code)
  );
...
function saveStay(e){
  ...
  /*
   * If somebody manually adds a planned trip
   * containing past dates, immediately split
   * passed dates into actual.
   */
...
function openGapDialog(
  ...
  els.gapDateSummary.textContent=
    `Fill ${fmt(start)} to ${fmt(end)} (${plural(daysInclusive(start,end),'day')}). The first and last dates are intentionally included so travel days can overlap with the country either side.`;


  els.gapCountry.value=
...
function populateProfileSelects(){
  ...
  els.plannerProfile.innerHTML=
    opts;


  els.stayProfile.innerHTML=
...
function openProfileDialog(
  ...
  els.profileDialogTitle.textContent=
    id
      ?'Edit profile'
      :'Add profile';


  els.deleteProfileBtn.classList.toggle(
    'hidden',
    !id||
...
function addCitizenship(){
  ...
  if(
    !profileCitizenships.includes(c.code)
  )
    profileCitizenships.push(
      c.code
    );
...
function renderCitizenshipChips(){
  ...
  els.citizenshipChips.innerHTML=
    profileCitizenships.map(
      c=>`
        <span class="chip">
...
function renderVisaChecker(){
  ...
  if(cit.length){
        ...
        code=>{
          ...
          return c
            ?`
              <button
                type="button"
                data-action="use-profile-passport"
                data-country="${code}"
              >
                ${
                  flagHtml(
    ...
    els.visaProfileHint.innerHTML=
      `No passport saved for ${esc(p?.name||'this traveller')}. You can still type one above, or <button type="button" data-go-view="profiles">add it to the profile</button>.`;
  }
  ...
  if(
    ...
    els.visaDataStatus.textContent=
      'Visa data will load when you run a check.';
}
...
async function loadVisaDataset(){
  ...
  if(visaDatasetPromise)
    return visaDatasetPromise;


  if(els.visaDataStatus)
    els.visaDataStatus.textContent=
      'Loading visa-policy data…';


  const url=
    'https://raw.githubusercontent.com/geetpurwar/countries_with_visa_and_flags/main/countries_with_visa_and_flags.min.json';
  ...
  visaDatasetPromise=
    ...
    .then(r=>{
      ...
      if(!r.ok)
        throw new Error(
          `Visa data request failed (${r.status})`
        );
    ...
    .then(data=>{
      ...
      if(els.visaDataStatus)
        els.visaDataStatus.textContent=
          `Quick checker ready · ${data.length} passport records loaded from the open visa-policy dataset.`;
    ...
    .catch(err=>{
      ...
      if(els.visaDataStatus)
        els.visaDataStatus.textContent=
          'Quick visa data could not be loaded. Use “Open live requirements” to check instead.';
...
function visaRequirementInfo(req=''){
  ...
  if(
    x==='eta'||
    x.includes('electronic travel')
  )
    return{
      tone:'warn',
      badge:'TRAVEL AUTHORISATION',
      title:'Electronic travel authorisation required',
      label:'ETA / electronic authorisation',
  ...
  if(
    x==='visa'||
    x.includes('visa required')
  )
    return{
      tone:'bad',
      badge:'VISA REQUIRED',
      title:'Visa required before travel',
      label:'Visa required',
      answer:'Yes'
    };
  ...
  if(
    x.includes('no admission')||
    x.includes('not admitted')
  )
    return{
      tone:'bad',
      badge:'RESTRICTED',
      title:'Entry may be restricted',
      label:req||'Restricted',
      answer:'Check'
    };
...
async function runVisaCheck(){
  ...
  let from=
      ...
      to=
        countryByName(
          els.visaDestination.value
        );


  if(!from||!to){

    els.visaResultTitle.textContent=
      'Check your countries';


    els.visaResultBody.innerHTML=
      '<div class="visa-error">Choose a valid passport country and destination from the country list.</div>';
  ...
  els.visaResultBody.innerHTML=
    '<div class="planner-placeholder">Loading the visa-policy data.</div>';


  if(from.code===to.code){

    els.visaResultTitle.innerHTML=
      `${flagHtml(from.code)} ${esc(from.name)} → ${flagHtml(to.code)} ${esc(to.name)}`;


    els.visaResultBody.innerHTML=`
      <div class="visa-result-card">

        <div class="visa-answer good">
          ...
          <h3>
            No entry visa needed as a citizen
          </h3>

          <p>
            This checker is for foreign-entry visa requirements.
            You may still need a valid passport or national identity
            document for travel.
          </p>
  ...
  try{
    ...
    if(!src||!dst)
      throw new Error(
        'Country not found in visa dataset'
      );


    let rule=
      (src.access_to_countries||[])
      .find(
        x=>
    ...
    if(!rule)
      throw new Error(
        'No passport/destination rule was found'
      );
    ...
    els.visaResultBody.innerHTML=`
      <div class="visa-result-card">
        ...
        <div class="visa-answer ${info.tone}">
          ...
          <p>
            For a short tourist visit,
            the quick dataset lists this route as
            <strong>${esc(info.label)}</strong>

            ${
              rule.days
                ?` with a stay of up to <strong>${rule.days} days</strong>`
                :''
            }.
        ...
        <div class="visa-detail-grid">
          ...
          <div class="visa-detail">
            ...
            <strong>${esc(info.answer)}</strong>
          </div>

          <div class="visa-detail">
            <span>Quick requirement</span>
            <strong>${esc(info.label)}</strong>
          </div>

          <div class="visa-detail">
            <span>Stay shown</span>
            <strong>${esc(stay)}</strong>
          </div>
        ...
        <div class="visa-actions">
          <a
            class="primary"
            href="https://apply.joinsherpa.com/visa"
            target="_blank"
            rel="noreferrer"
          >
    ...
    els.visaResultBody.innerHTML=`
      <div class="visa-error">
        ...
        Use the live requirements checker below before relying on an answer.
      </div>
...
function runPlanner(){
  ...
  let p=
        state.profiles.find(
          x=>
            x.id===
            els.plannerProfile.value
        )||
        activeProfile(),

      c=
        countryByName(
          els.plannerCountry.value
        ),
      ...
      exit=
        els.plannerExit.value;


  lastPlannerTrip=null;


  els.addPlannedTripBtn.classList.add(
    'hidden'
  );
  ...
  if(!c||!entry){
    ...
    els.plannerResultTitle.textContent=
      'Add a destination and entry date';
  ...
  if(SCHENGEN.has(c.code)){
    ...
    if(
      ...
      els.plannerResultTitle.textContent=
        'Schengen 90/180 does not apply to this profile';
    ...
    els.plannerResultTitle.innerHTML=
      `${flagHtml(c.code)} ${esc(c.name)} · Schengen`;
      ...
      >
        ...
        ${
          exit
            ?(
              over
                ?`Your proposed trip would first exceed the 90/180 limit on <strong>${fmt(over.date)}</strong>. Shorten the trip or change the dates.`
                :`Your proposed ${daysInclusive(entry,exit)}-day trip stays within the recorded 90/180 history.`
            )
            :max
            ?`If you enter on <strong>${fmt(entry)}</strong>, your current records allow up to <strong>${max} consecutive days</strong>, leaving by <strong>${fmt(safeEnd)}</strong>.`
            :`Your recorded history does not leave a Schengen day available on ${fmt(entry)}. Earliest one-day entry found: <strong>${reentry?fmt(reentry):'not within the next 2 years'}</strong>.`
        }
  ...
  if(rule){
    ...
    els.plannerResultBody.innerHTML=`
      ...
      <div class="result-message warn">
        ${esc(rule.desc)}
        This is only a planning template;
        verify that you are eligible and check
        the actual permission granted to you.
      </div>
    `;
...
function renderMapTimeline(){
  ...
  els.timelineSlider.dataset.start=
    min;
        ...
        return`
          <div
            ...
            style="
              ...
              top:${top}px
            "
            title="${esc(s.countryName)}: ${fmt(s.start)} – ${fmt(s.end)}"
          ></div>
        `;
      }
    ).join('');
...
function updateTimelineLabels(){
  ...
  let on=
    state.stays.filter(
      s=>
        s.start<=timelineDate&&
        s.end>=timelineDate
    );


  els.timelineDateLabel.textContent=
    fmt(
      timelineDate,
      {
        weekday:'long',
  ...
  els.timelineLocationLabel.textContent=
    on.length
      ?on.map(
        s=>
          `${s.countryName}${s.status==='planned'?' (planned)':''}`
      ).join(' · ')
  ...
  els.timelineSchengenLabel.textContent=
    isSchengenExemptProfile()
      ?'Not applied'
      :`${r.used} / 90`;


  els.mapSelectionSummary.textContent=
    on.length
...
function installDataTransferUI(){
  ...
  let nav=
    document.querySelector(
      '.nav-item[data-view="profiles"]'
    );


  if(nav)
    nav.innerHTML=
      '<span>♙</span>Profiles & data';
  ...
  if(importLabel){
    ...
    if(input){
      ...
      input.setAttribute(
        'accept',
        '.travel,.json,application/json'
      );


      importLabel.appendChild(
        input
      );
    }
  }
  ...
  if(footer)
    footer.textContent=
      'Your travel data is automatically saved on this device.';
  ...
  profiles.insertAdjacentHTML(
    ...
    `
      <article
        class="panel data-transfer-panel"
        id="dataTransferPanel"
      >
        ...
        <div class="transfer-grid">
          ...
          <div class="transfer-card">
            ...
            <p>
              Create a temporary, one-use code
              containing an encrypted copy
              of your travel data.
            </p>
            ...
            <div
              id="transferCodeArea"
              class="transfer-code-area hidden"
            >
              ...
              <strong
                id="transferCodeValue"
                class="transfer-code-value"
              >
                —
              </strong>
          ...
          <div class="transfer-card">
            ...
            <p>
              Open this website on the new
              device and enter the temporary
              code from your old device.
            </p>
        ...
        <div
          id="transferMessage"
          class="form-message transfer-message"
        ></div>
        ...
        <div class="backup-strip">
          ...
          <div>
            ...
            <span>
              Useful if you clear browser
              data, lose a device, or want
              a permanent copy.
            </span>
        ...
        <div class="privacy-note">
          ...
          <span>
            Transfer data is encrypted in
            your browser before upload.
            The temporary code expires after
            one hour and is single-use.
...
function updateLocalSaveIndicator(){
  ...
  if(!raw){
    ...
    detail.textContent=
      'Your changes save automatically on this device.';
...
function transferServiceConfig(){
  ...
  let url=
        (
          TRANSFER_SERVICE.supabaseUrl||
          ''
        ).trim(),

      key=
        (
          TRANSFER_SERVICE.supabaseKey||
          ''
        ).trim(),
...
function getTransferClient(){
  ...
  if(!cfg.valid)
    throw new Error(
      'Device transfer has not been configured by the site owner yet.'
    );


  if(!window.supabase)
    throw new Error(
      'The transfer service is still loading. Try again in a moment.'
    );
...
async function decryptTransferPayload(
  code,
  encryptedPayload,
  ivB64
){
  ...
  if(
    payload?.format!==
      'where-ive-been-transfer'||
    !payload.state
  )
    throw new Error(
      'This transfer could not be read.'
    );
...
function setTransferMessage(
  ...
  if(!els.transferMessage)
    return;


  els.transferMessage.textContent=
    message||'';
...
function setTransferBusy(busy){
  ...
  if(els.createTransferBtn)
    els.createTransferBtn.disabled=
      busy;


  if(els.claimTransferBtn)
    els.claimTransferBtn.disabled=
      busy;
}
...
async function createTransferCode(){
  ...
  try{
    ...
    els.transferCodeValue.textContent=
      lastTransferCode;


    els.transferCodeArea
      .classList.remove(
        'hidden'
      );
...
function exportData(){
  ...
  a.download=
    `Where-Ive-Been Backup - ${isoDate(new Date())}.travel`;
...
function setCloudUI(
  ...
  els.cloudStatusBadge.textContent=
    label;
  ...
  els.cloudPullBtn.disabled=
    !signed;


  els.cloudPushBtn.disabled=
    !signed;
