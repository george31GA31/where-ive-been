...
(() => {
  ...
  function render(){if(!ready)return;renderAchievements();renderYear();renderTransportList();if(document.body.dataset.currentView==='country'||location.hash.includes('/country/'))renderCountry();renderHomeSummary();renderMapSelection();}
  ...
  function init(){
    ...
    const style=document.createElement('link');style.rel='stylesheet';style.href=new URL('journeys.css?v=redesign-1',root);document.head.append(style);
    ...
    refresh();
    for(const key of ['capitals',...Object.keys(J.categories)])fetch(new URL('data/'+key+'.json?v=redesign-1',root)).then(r=>{if(!r.ok)throw Error(r.status);return r.json();}).then(data=>{catalog[key]=data;render();renderMap();}).catch(()=>{failures.add(key);render();});
  }
